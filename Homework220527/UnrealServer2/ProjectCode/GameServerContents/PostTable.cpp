...
PostTable_SelectIDToPostInfo::PostTable_SelectIDToPostInfo(std::string _ID)
	: DBQuery("SELECT Idx, ToNickName, Letters, FromNickName FROM userver2.post WHERE ToNickName = ?")
	, ToNickName(_ID)
{
}
...
bool PostTable_SelectIDToUserInfo::DoQuery()
{
	...
	RowData = std::make_shared<PostRow>(Result->GetInt(0), Result->GetString(1), Result->GetString(2), Result->GetString(3));
	return true;
}
...
bool PostTable_AllUserInfo::DoQuery()
{
	...
	while (Result->Next())
	{
		RowDatas.push_back(std::make_shared<PostRow>(Result->GetInt(0), Result->GetString(1), Result->GetString(2), Result->GetString(3)));
	}
	return true;
}
...
PostTable_InsertUserInfo::PostTable_InsertUserInfo(std::string _ID, std::string _Letters, std::string _FromNickName)
	: DBQuery("INSERT INTO userver2.post (ToNickName, Letters, FromNickName) VALUES (?, ?, ?)")
	, ToNickName(_ID)
	, Letters(_Letters)
